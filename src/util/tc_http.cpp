...
bool TC_Http::checkHeader(const char *sHeader, const char *value) const
{
	...
	return ::strcasecmp(it->second.c_str(), value) == 0;
}
...
size_t TC_HttpResponse::parseResponseHeaderString(const char *beginIt, const char *headerIt)
{
	...
	if(::strncasecmp(_headerLine.c_str(), "HTTP/", 5) != 0)
	{
...
PACKET_TYPE TC_HttpRequest::checkHttp(TC_NetWorkBuffer&in)
{
    try
    {
	    bool b = TC_HttpRequest::checkRequest(in);
...
PACKET_TYPE TC_HttpRequest::parseHttp(TC_NetWorkBuffer&in, vector<char> &out)
{
    PACKET_TYPE b = checkHttp(in);
	...
	while (true)
	{
		...
		if(!first)
		{
			...
			if (itF != NULL)
			{
				...
				if (::strncasecmp(name.c_str(), "Content-Length", 14) == 0)
				{
...
void TC_HttpRequest::parseRequestHeader(const char* szBuffer, const char *header)
{
    ...
    if (::strncasecmp(f1 + 1, "https://", 8) != 0 )
    {
        if (::strncasecmp(f1 + 1, "http://", 7) != 0 )
        {
